...
#ifndef AnalyserDBsKsKs_h
#define AnalyserDBsKsKs_h
...
public :
   ...
   AnalyserDBsKsKs(TTree *tree=0);
   virtual ~AnalyserDBsKsKs();
...
#ifdef AnalyserDBsKsKs_cxx
AnalyserDBsKsKs::AnalyserDBsKsKs(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("DTT_2016_Reco16Strip28_Down_BHADRON.root");
      if (!f || !f->IsOpen()) {
...
AnalyserDBsKsKs::~AnalyserDBsKsKs()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t AnalyserDBsKsKs::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t AnalyserDBsKsKs::LoadTree(Long64_t entry)
{
...
void AnalyserDBsKsKs::Init(TTree *tree)
{
...
void AnalyserDBsKsKs::Show(Long64_t entry)
{
...
Int_t AnalyserDBsKsKs::Cut(Long64_t entry)
{
...
#endif // #ifdef AnalyserDBsKsKs_cxx
